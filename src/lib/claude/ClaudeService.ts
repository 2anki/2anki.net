...
// The conversion model, in one place: the string sent to the API AND the string
// folded into the cache key must be identical, so a model swap invalidates the
// cache automatically instead of serving decks generated by the previous model.
export const CONVERSION_MODEL = 'claude-sonnet-5';

// A digest of the system prompt. Folded into the cache key so any edit to
// SYSTEM_PROMPT (a prompt bump) changes every key and retires the cached decks
// the previous prompt produced — no manual version bump, no stale output.
...
// The vision path reads image bytes off disk that the cache key does not
// capture, so it is never cached — only the text path, whose entire input is
// the HTML plus the settings folded into the key, is safe to serve from cache.
export async function generateDeckInfo(
  ...
  const store = options?.conversionResultCache;
  if (store == null || options?.pdfImageFallback != null) {
    return compute();
  }

  return withConversionResultCache(
    store,
    {
      content: htmlContent,
...
async function generateDeckInfoUncached(
  htmlContent: string,
  availableMediaFiles: string[],
  userInstructions?: string,
  onProgress?: (step: string) => void,
