...
- When a unit of work is done, ship it: commit, push, open the PR with `gh pr create` (not `--draft`), then run **`/ship <n>`** — it runs the review agent, waits for the gate, merges, watches the deploy, and verifies prod. Reserve `--draft` for work that genuinely isn't reviewable yet: a `/spec-draft-pr` spec awaiting `/implement`, or a WIP you've explicitly flagged as WIP.
