# SonarCloud — state, setup, false positives

Moved out of `.claude/rules/sonar.md` (2026-08-25) so it loads only when a Sonar task needs it. The merge-time rule (zero open findings) still lives in the rules file.
...
## Existing rule waivers

The waiver set (`sonar.issue.ignore.multicriteria.*`) and exclusions live in `sonar-project.properties` and `.sonarcloud.properties` — read those files, don't copy them here. `src/lib/sonarConfigParity.test.ts` enforces parity between the two; `src/data_layer/public/**` is excluded outright (Kanel-generated).
