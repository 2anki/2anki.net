...
- **A leftover worktree under `.claude/worktrees/` used to double and redden the main checkout's full-suite run.** Jest's `testMatch` is `**/*.test.ts`, so a worktree holding a second copy of the repo got swept in: every suite ran twice, and the worktree copy ran without the Python venv and `pdfinfo`, so a clean tree reported ~365 failed suites out of 1664 that had nothing to do with the branch under test. `tsc -p .` had the same blind spot. Both configs now exclude `.claude/` (`jest.config.js` `testPathIgnorePatterns`, `tsconfig.json` `exclude`), so this is fixed rather than remembered. Still remove finished worktrees — the doubled run is gone, but the disk and orphaned processes are not.
